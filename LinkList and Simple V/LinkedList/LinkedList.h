...
class LinkedList{
    ...
    public:
        ...
        void addAfter(const T&, int);
        void addBefore(const T&, int);
        void addBeg(const T&);
...
template<class T>
void LinkedList<T>::addAfter(const T &data, int lnk){
    lnk--;
    next=front;
    for(int i=0;i<lnk;i++)
        next=next->linkPtr;
    Link* add=new Link;
    add->data=data;
    add->linkPtr=next->linkPtr;
    next->linkPtr=add;
    
}

template<class T>
void LinkedList<T>::addBefore(const T &data, int lnk){
    next=front;
    lnk--;
...
template<class T>
void LinkedList<T>::addBeg(const T &data){
    Link* add=new Link;
    add->data=data;
    add->linkPtr=front;
    front=add;
